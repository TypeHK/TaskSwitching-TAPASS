...
function nextSlide(){
   ...
   if (curr == "switch-final" || curr == "dualtask-final"){
      disableSpacebar()
      window.location.href = 'https://www.test.uva.nl/lotus/TAPASS/event/end_switch_task/' + userID // Task End
...
function populateScore(){
    ...
    $('.' + slideOrder[slideCounter] + ' .validityRelativeResultDual').html(msval.toFixed(0))
...
function completeBlock(){
   blockCounter++;
   userData['blockCounter'] = blockCounter;
   console.dir(userData) // Data sent to Lotus
   $.ajax({
      url: "/lotus/TAPASS/event/store_data/" + userID,
      type: "POST",
      data: userData
   });
   nextSlide()
